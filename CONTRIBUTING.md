# Contributing to dmf-env

Generic environment tooling for the DMF Platform: `bin/` scripts, `terraform/modules/` + generic per-provider roots, neutral `tasks/`/`templates/`. Per-env state is operator-local and never committed (ADR-0035).
...
(The full pre-publish history lives in a LAN-only `dmf-env-archive` Forgejo
...
- Keep the surface generic — per-env state stays under `~/.dmfdeploy/envs/<env>/`;
  placeholder syntax everywhere.
